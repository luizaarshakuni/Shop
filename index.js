...
function createNavItems() {
  navItems.forEach((item, index) => {
    if (index < navItems.length - 2) {
      ...
      p.textContent = item;
      navigation.append(p);
    } else if (index === navItems.length - 2) {
...
basketIcon.addEventListener("click", () => {
  basketDiv.classList.toggle("activeBasket");
});

//*****Categories and brands
const categories = products_data.reduce(
  (acc, currentItem) =>
    acc.includes(currentItem.category) ? acc : [...acc, currentItem.category],
  ["All"]
);

const brands = products_data.reduce(
  (acc, currentItem) =>
    currentItem.brand
      ? acc.includes(currentItem.brand)
        ? acc
        : [...acc, currentItem.brand]
      : acc,
  ["All"]
);
...
function leftMenuItems() {
  ...
  pricing.innerHTML = `
  <label>Up to : </label>
  <input type="number" class="maxprice" placeholder="Enter max price" min="0" />`;
  leftMenu.append(pricing);

  const categoryDiv = document.createElement("div");
  categoryDiv.classList.add("categoryDiv");

  categoryDiv.innerHTML += `<h1>Category</h1>`;
  categories.forEach((categoryName) => {
    categoryDiv.innerHTML += `<p onclick="chooseCategory('${categoryName}')">${categoryName}</p>`;
  });
  leftMenu.append(categoryDiv);

  const brandDiv = document.createElement("div");
  brandDiv.classList.add("brandDiv");

  brandDiv.innerHTML += `<h1>Brand</h1>`;
  brands.forEach((brandName) => {
    brandDiv.innerHTML += `<p onclick="chooseBrand('${brandName}')">${brandName}</p>`;
  });
  leftMenu.append(brandDiv);
}
leftMenuItems();
  ...
  priceInput.addEventListener("input", () => {
    ...
    if (maxPrice !== "") {
      const filtered = products_data.filter(
        (prod) => prod.price <= Number(maxPrice)
      );
      showAllProducts(filtered);
    } else {
      showAllProducts(products_data);
    }
  });
}
...
function showAllProducts(arr) {
  const allProductsSection = document.querySelector(".allProducts");
  ...
  arr.forEach((prod) => {
    ...
    <div>
        ...
        <p>${prod.title}</p>
        <p>${prod.brand ? prod.brand : "None"}</p>
        <p>${prod.category}</p>
        <p><b>${prod.price}$</b></p>
        <button onclick="createBasketElement(${prod.id})">Add to Cart</button>
    </div>`;
  });
}
showAllProducts(products_data);
...
function chooseCategory(text) {
  ...
  const filtered = products_data.filter((prod) => prod.category === text);
  if (filtered) {
    showAllProducts(filtered);
  }
...
function chooseBrand(text) {
  if (text === "All") {
    showAllProducts(products_data);
    return;
  }
  const filtered = products_data.filter((prod) => prod.brand === text);
  if (filtered) {
    showAllProducts(filtered);
  }
...
function createBasketElement(id) {
  const currentProduct = products_data.find((prod) => prod.id === id);

  if (basket.includes(currentProduct)) {
...
function showBasketElement() {
  ...
  if (basket.length === 0) {
    basketDiv.innerHTML = `<h5>The basket is empty !</h5>`;
  ...
  basket.forEach((prod) => {
    ...
    basketDiv.innerHTML += `
      <div class="basket_el">
        ...
        <i class="fa-solid fa-minus" onclick="plusORminus(${
          prod.id
        }, 'minus')"></i>
        <h4>${prod.count}</h4>
        <i class="fa-solid fa-plus" onclick="plusORminus(${
          prod.id
        }, 'plus')"></i>
        </div>
        <h4>${(prod.price * prod.count).toFixed(2)}$</h4>
      </div>`;
  });
...
//*****Plus and Minus
function plusORminus(id, operator) {
...
//***** Search
const search = document.querySelector(".inputField input");

search.addEventListener("input", () => {
  const searchingItem = search.value.toLowerCase().trim();
  ...
  const filteredProducts = products_data.filter((product) =>
    product.title.toLowerCase().includes(searchingItem)
  );
